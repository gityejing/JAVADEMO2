...
public class JavadocDemo {
	...
	public static boolean mathOperator() {
		...
		int intRnd = (new Random()).nextInt(Integer.MAX_VALUE % 1000);// 等同于这个rnd.nextInt(10);
		System.out.println(intRnd);
		double dblRnd = rnd.nextLong() * intRnd;// 整形的要在括号里面给种子// long, float
		...
		if (isSuccess) {
			// 分割用正则表达式
			char chrArr[] = int2toBinaryString(intRnd).toCharArray();// 将整形转为二进制再给字符数组
	...
	public static void bitOperator() {
		...
		do {
			...
			pln(intNum + "  的原码值:" + String.format("%025d", 0)
                    + int2toBinaryString(intNum));
			pln(intNum + "  的反码值:" + String.format("%025d", 0)
                    + int2toBinaryString(intNum));
			pln(intNum + "  的补码值:" + String.format("%025d", 0)
                    + int2toBinaryString(intNum));
			pln("负数的原码最高位为1、反码(在其原码的基础上, 符号位不变，其余各个位取反)、\n补码(在其原码的基础上, 符号位不变, 其余各位取反, 最低位+1)");
			pln((-intNum) + " 的原码值:1" + String.format("%024d", 0)
                    + int2toBinaryString(intNum));
			pln((~intNum + 1) + " 的反码值:" + int2toBinaryString((~intNum) + 1));
			pln((~intNum + 1) + " 的补码值:" + int2toBinaryString((~intNum) + 2));

			pln("位移的运算   1.左移后，低位会被自动补零（0）  2.右移如果原值是正数，则高位补上0；如果原值是负数，高位补1。");

			pln(intNum + "位移操作前的二进制码是       " + int2toBinaryString(intNum));
	...
	public static void systemOperator() {
		...
		for (; sysIt.hasNext();) {
			Entry<Object, Object> sysEt = (Entry<Object, Object>) sysIt
					.next();
	...
	public static String int2toBinaryString(int x) {
